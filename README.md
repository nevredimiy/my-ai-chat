# AloChat (RAG-бот для WordPress и WooCommerce)
...
Настройки хранятся в опциях WordPress с префиксом `alochat_`.
...
Плагин слушает `save_post` и `woocommerce_update_product`. При публикации/обновлении записи, страницы или товара ставится фоновая задача `alochat_index_single_post_cron`:
...
```bash
# Локально
wp alochat index --batch_size=50

# WordPress в Docker
docker exec -it wp_php_wp_68 php wp-cli.phar alochat index --batch_size=50 --allow-root
```
...
docker exec -it wp_php_wp_68 php wp-cli.phar alochat search "червона кепка" --allow-root

# Полный RAG-цикл: вопрос → поиск → генерация ответа LLM
docker exec -it wp_php_wp_68 php wp-cli.phar alochat ask "Я шукаю червону кепку, у вас є?" --allow-root
```
...
- `POST /wp-json/alochat/v1/chat` (или `/index.php?rest_route=/alochat/v1/chat` без ЧПУ)
...
Диагностические сообщения пишутся в лог PHP **только при включённом `WP_DEBUG`** (функция `alochat_log()`).
...
* **404 на `/wp-json/...`** — отключены ЧПУ; используйте `/index.php?rest_route=/alochat/v1/chat`.
...
```bash
docker exec wp_php_wp_68 php wp-cli.phar plugin check alochat --allow-root
```
